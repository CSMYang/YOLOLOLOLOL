...
if __name__ == "__main__":
    for count in range(2):
        ...
        #     filter(lambda p: p.requires_grad, yolo.parameters()), lr=init_lr, weight_decay=weight_decay)
        ...
        for iteration in range(training_cycle):
            ...
            if count == 0:
                torch.save(yolo.state_dict(),
                           "data\\training_result\\8\\5\\latest_state.pth")
            else:
                torch.save(yolo.state_dict(),
                           "data\\training_result\\latest_state.pth")
            if best_loss == 0 or best_loss >= total_val_loss:
                ...
                if count == 0:
                    torch.save(yolo.state_dict(),
                               "data\\training_result\\8\\5\\best_state.pth")
                else:
                    torch.save(yolo.state_dict(),
